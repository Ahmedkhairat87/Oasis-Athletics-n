...
import '../../../../../../core/model/stdLinks/academicSupport/ReportNumbers.dart';
...
class _AcademicTabState extends State<AcademicSupportMainTab> {
  ...
  Future<void> _openReport(int reportNo) async {
    final stdId = studentNotifier.value.stdId.toString();

    setState(() => loading = true);

    try {
      final reportResponse = await AcademicSupportService.getAcademicSupportReport(
        stdId: stdId,
        reportNo: reportNo,
      );
      ...
      if (reportResponse == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("No report data found")),
        );
        return;
      }
  ...
  Widget _studentsReportsCard(
    ...
    final List<ReportNumbers> reports = responseData?.reportNumbers ?? [];
    ...
    return Column(
      children: List.generate(reports.length, (index) {
        ...
        final int reportNumber = (report.reportNo ?? 0).toInt();
        final String reportTitle =
        (report.reportDesc != null && report.reportDesc!.trim().isNotEmpty)
            ? report.reportDesc!
            : "Report $reportNumber";
        ...
        return Padding(
          ...
          child: AnimatedContainer(
            ...
            child: InkWell(
              borderRadius: BorderRadius.circular(12.r),
              onTap: () => _openReport(reportNumber),
              child: Row(
                children: [
                  Icon(
                    Icons.description_rounded,
